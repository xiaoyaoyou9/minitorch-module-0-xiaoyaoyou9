...
def test_symmetric() -> None:
    ...
    # TODO: Implement for Task 0.2.
    # raise NotImplementedError("Need to implement for Task 0.2")
    assert is_close(mul(3.0, 4.0), mul(4.0, 3.0))
...
def test_other() -> None:
    ...
    # raise NotImplementedError("Need to implement for Task 0.2")
    x, y = 5.0, 10.0
    assert is_close(add(x, y), add(y, x))
    assert is_close(mul(x, y), mul(y, x))
    assert is_close(add(x, add(y, 1.0)), add(add(x, y), 1.0))
    assert is_close(mul(x, mul(y, 2.0)), mul(mul(x, y), 2.0))
